      ...
      {isLoading ? (
        ...
        <div className="surface-card flex flex-col items-center justify-center px-6 py-20 text-center">
          ...
          <p className="mt-1 max-w-sm text-caption text-slate-500 dark:text-slate-400">
            {L("جرّب تعديل رقم اللوحة أو الفلاتر.", "Try adjusting the plate number or filters.")}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filteredVehicles.map((v) => (
            <div
              key={v.id}
              className="surface-card surface-card-hover space-y-4 p-5 transition-transform duration-200 hover:-translate-y-0.5"
            >
