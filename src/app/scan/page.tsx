      ...
      const verifyToken = async () => {
        ...
        try {
          ...
          if (res.ok && data.success && data.vehicle) {
            ...
            setErrorMessage(data.error || "تصريح الموقف غير صالح أو منتهي الصلاحية");
          }
        } catch {
          setErrorMessage("حدث خطأ أثناء فحص تصريح الموقف");
        } finally {
          setIsLoading(false);
        }
      };
    ...
    try {
      const res = await fetch("/api/scan/alert", {
        ...
        body: JSON.stringify({
          ...
          message: selectedReason === "BLOCKING" ? "سيارتك حاجزة سيارتي يرجى التكرم بتحريكها" : undefined,
        }),
      });
      ...
      if (res.status === 429) {
        setRateLimitWarning(json.error || "تم إرسال تنبيه مسبقاً لهذه السيارة. يرجى الانتظار 5 دقائق منعاً للإزعاج.");
        triggerHaptic("error");
        return;
      }
      ...
      if (res.ok && json.success) {
        ...
        setRateLimitWarning(json.error || "تعذر إرسال التنبيه حالياً");
        triggerHaptic("error");
      }
    } catch {
      setRateLimitWarning("حدث خطأ في الاتصال بالخادم");
      triggerHaptic("error");
    } finally {
      setIsSending(false);
    }
    ...
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-sm font-bold text-slate-600 dark:text-zinc-400">
          جارٍ التحقق من تصريح الموقف المشفر عبر النظام الآمن...
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="mx-auto max-w-md p-6 space-y-6 text-center animate-in fade-in duration-300">
        ...
        <div className="space-y-3">
          ...
          >
            <span>تسجيل دخول كادر المنشأة</span>
          </Link>
          <Link
            href="/"
    ...
    <div className="mx-auto max-w-md p-4 space-y-5 animate-in fade-in duration-300">
      ...
      <div className="text-center pt-2">
        <div className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-300 mb-2">
          ...
          <span>تصريح موقف ذكي معتمد</span>
        </div>
        <h1 className="text-xl font-black text-slate-900 dark:text-white font-arabic">
          تنبيه سريع لسائق المركبة
        </h1>
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
          تم التعرف على تصريح المركبة المشفر بنجاح
        </p>
      </div>
      ...
      <div className="rounded-3xl border border-slate-200/80 bg-white p-5 shadow-lg dark:border-zinc-800 dark:bg-[#0c0c0f]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-slate-500 dark:text-zinc-400">
                اللون: <span className="font-bold text-slate-700 dark:text-zinc-300">{vehicle.color}</span>
              </div>
            </div>
          </div>
        ...
        <div className="pt-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              { id: "BLOCKING", label: "سيارتك تغلق مساري 🚗" },
              { id: "LIGHTS_ON", label: "أنوار السيارة مشتعلة 💡" },
              { id: "WINDOW_OPEN", label: "نافذة السيارة مفتوحة 🪟" },
              { id: "CONTACT_ME", label: "يرجى تحريك المركبة 🅿️" },
            ].map((r) => (
              <button
                key={r.id}
                type="button"
      ...
      {isAlertSent ? (
        <div className="rounded-3xl bg-emerald-50 border border-emerald-200 p-6 text-center animate-in zoom-in-95 duration-200 dark:bg-emerald-950/40 dark:border-emerald-900">
          ...
          <h3 className="text-lg font-black text-emerald-900 dark:text-emerald-200 font-arabic">
            تم إرسال التنبيه فورياً للسائق!
          </h3>
          <p className="text-xs text-emerald-700 dark:text-emerald-400 mt-1">
            وصل إشعار مباشر على شاشة هاتفه لتحريك المركبة بأسرع وقت.
          </p>
        ...
        <div className="space-y-2.5">
          ...
          >
            ...
            <span>{isSending ? "جارٍ إرسال التنبيه..." : "إرسال تنبيه فوري للسائق (بنقرة واحدة)"}</span>
          </button>
        </div>
      )}
